...
(function () {
  ...
  function buildOne(box) {
    ...
    // dagre reserves no space for edge labels, but the edge style wraps them at
    // text-max-width EDGE_LABEL_W. With the default rank gap the label is wider than
    // the space between ranks and smears across the nodes on either side, so widen the
    // gap whenever any edge carries a label.
    var EDGE_LABEL_W = 120;
    var rankDir = spec.rankDir || "TB";
    var acrossRanks = (rankDir === "LR" || rankDir === "RL");
    var hasEdgeLabel = (spec.elements || []).some(function (el) {
      return el.data && el.data.source && el.data.label;
    });
    var rankSep = spec.rankSep || (graphmap ? 84 : 46);
    if (hasEdgeLabel) {
      rankSep = Math.max(rankSep, acrossRanks ? EDGE_LABEL_W + 40 : 96);
    }
    var layoutOpts = {
