...
이 한 줄의 금지 규칙이 곧 주기율표의 골격이다. 1s에 2개, 2s에 2개, 2p에 6개(스핀 2 × 모드 3)로 칸이 차면 전자는 더 이상 들어갈 자리가 없어 다음 줄로 넘어간다. 우리가 외운 주기율표의 *행(period) 구조*가 바로 이 "한 오비탈에 둘까지"라는 Pauli 제약의 직접적 결과다. 만약 Pauli가 없었다면 모든 전자가 가장 낮은 1s로 우르르 떨어졌을 것이고, 원소마다 다른 전자 배치도, 그 배치가 만드는 화학적 개성도 사라진다. 곧 화학 자체가 존재하지 않는다.
...
그리고 이 교환 에너지는 원자 한 개에서 끝나지 않는다. 같은 "평행 스핀이 에너지를 낮춘다"는 원리가 결정 전체로 확장되면, 수많은 원자의 홀전자 스핀이 한 방향으로 줄을 맞추려는 경향이 생긴다. 이것이 철, 코발트, 니켈이 영구 자석이 되는 **강자성(ferromagnetism)** 의 미시적 기원이다. 즉 우리가 손에 쥐는 자석의 거시적 자성은, 한 원자 안에서 Hund 규칙을 만든 바로 그 교환 에너지가 거시 규모로 증폭된 결과다. 한 줄로 정리하면, 강자성은 교환 에너지의 거시 확장이다.
