...
![메탄의 정사면체 기하](https://upload.wikimedia.org/wikipedia/commons/thumb/c/cd/Methane_tetrahedral_lines.svg/960px-Methane_tetrahedral_lines.svg.png)

*메탄 CH₄ 의 정사면체 기하. 네 C–H 결합이 모두 동등하게 109.5° 로 벌어진다.*
*바닥 상태의 비대칭 s·p 오비탈로는 설명되지 않는 이 완전한 대칭이 곧 혼성을 가리키는 단서다.*
...
![다이아몬드 입방 격자](https://upload.wikimedia.org/wikipedia/commons/thumb/3/3e/NCN-cubic-diamond-xtal-3D-balls.png/960px-NCN-cubic-diamond-xtal-3D-balls.png)

*다이아몬드의 입방 격자. 모든 탄소가 sp³ 혼성으로 네 이웃과 정사면체 방향 공유 결합을 이루어 3차원 그물을 만든다.*
*끊어지지 않고 사방으로 이어진 이 방향성 강결합 그물이 초경질과 절연성을 동시에 낳는다.*
*출처: Ben Mills, PD (Wikimedia Commons).*
