...
```mermaid
graph TD
    A["linalg.matmul<br/><i>고수준: '행렬 곱셈'이라는 의미 보존</i>"] -->|"Tiling, Fusion 최적화"| B["linalg.matmul (tiled)<br/><i>고수준 유지: 타일링/퓨전 후에도 의미 보존</i>"]
    B -->|"Bufferization<br/>(tensor → memref)"| C["linalg on memref<br/><i>중수준: 메모리 접근 명시적</i>"]
    C -->|"Lower to Loops"| D["affine.for / scf.for + memref<br/><i>루프 구조 노출, 폴리헤드럴 분석 가능</i>"]
    D -->|"LLVM Lowering"| E["llvm.func + llvm.load/store<br/><i>LLVM IR: 기계 코드 생성 준비</i>"]
